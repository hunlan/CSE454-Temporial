...
import sf.filler.regex.RegexPerDateOfDeathFiller;
...
public class Assignment1 {
	public static void main(String[] args) throws Exception {
		...
		if (run) {
			...
			Filler filler = new RegexPerDateOfDeathFiller();
			//Filler filler = new RegexPerDateOfBirthFillerTwo();
			//Filler filler = new RegexOrgCountryOfHeadquartersFiller();//new
				...
				for (SFEntity query : queryReader.queryList) {
					if (query.answers.containsKey(filler.slotName)) {
						...
						answersString.append(String.format(
								"%s\t%s\t%s\t%s\t%s\n", query.queryId,
								filler.slotName, filler.getClass().getName(),
								ans.doc, ans.answer));
					} else {
						answersString.append(String.format(
								"%s\t%s\t%s\t%s\t%s\n", query.queryId,
